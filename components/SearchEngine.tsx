...
export default function SearchEngine() {
  ...
  const renderCalendar = (isDepart: boolean) => {
    ...
    return (
      <div className="bg-white rounded-sm shadow-2xl p-4 w-80 z-[9999] relative border border-gray-200">
        {/* Header with "Choose the Departure" or "Choose the Return" */}
        <div className="text-center mb-3">
          <span className="text-xs font-semibold text-[#AF935B] tracking-wider uppercase">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          >
            <ChevronLeft size={18} className="text-[#725B29]" />
          </button>
          ...
          <button
            ...
            className="p-1 hover:bg-gray-100 rounded-full transition-colors"
          >
            <ChevronRight size={18} className="text-[#725B29]" />
          </button>
        </div>
        
        <div className="grid grid-cols-7 gap-1 mb-2">
          {['SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'].map((day) => (
            <div key={day} className="text-center text-xs font-bold text-[#725B29] py-1">
              {day}
            </div>
          ))}
        </div>
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isPast ? 'text-gray-300 cursor-not-allowed line-through' : 'hover:bg-[#AF935B]/20'}
                  ${isSelected ? 'bg-[#AF935B] text-white hover:bg-[#725B29]' : ''}
                  ${isToday && !isSelected ? 'border-2 border-[#AF935B] text-[#725B29] font-bold' : ''}
                `}
              >
                {day}
              </button>
  ...
  return (
    <div className="bg-white rounded-sm shadow-2xl max-w-6xl mx-auto p-4 relative z-[100]">
      {/* Trip Type Toggle */}
      <div className="flex items-center gap-1 mb-4">
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 ${
            tripType === "roundtrip"
              ? "bg-[#AF935B] text-white shadow-lg shadow-[#AF935B]/30"
              : "text-[#111111]/60 hover:text-[#111111] hover:bg-gray-100"
          }`}
        >
          Round Trip
        </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
        ...
        <div className="relative lg:col-span-1" ref={leavingRef}>
          <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#AF935B] z-10">
          ...
          <input
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-sm pl-10 pr-8 py-3 text-[#111111] placeholder-gray-500 focus:outline-none focus:border-[#AF935B] focus:ring-2 focus:ring-[#AF935B]/20 transition-all duration-300 text-sm h-[52px]"
          />
          {leavingFrom && (
            <button
              onClick={() => {
          ...
          {showLeavingHints && filteredLeaving.length > 0 && (
            <div className="absolute bottom-full left-0 w-[280px] mb-1 bg-white rounded-sm shadow-lg border border-gray-200 max-h-52 overflow-y-auto z-[9999] custom-scrollbar">
              {filteredLeaving.map((loc) => (
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <MapPin size={14} className="text-[#AF935B] flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-medium text-[#111111]">
                        {loc.name}
                      </span>
                      <span className="text-xs text-[#111111]/50 ml-2">
                        {loc.country}
                      </span>
                    </div>
                  </div>
                  <div className="text-xs font-semibold text-[#725B29] bg-[#AF935B]/10 px-3 py-1 rounded-full flex-shrink-0 ml-2">
                    {loc.code}
                  </div>
                </button>
              ))}
        ...
        <div className="relative lg:col-span-1" ref={departCalendarRef}>
          ...
          <button
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-sm pl-10 pr-8 py-3 text-[#111111] text-left focus:outline-none focus:border-[#AF935B] focus:ring-2 focus:ring-[#AF935B]/20 transition-all duration-300 text-sm h-[52px]"
          >
            {departDate ? formatDisplayDate(departDate) : <span className="text-gray-500">Departure Date</span>}
          </button>
          {departDate && (
            <button
              onClick={() => {
        ...
        {tripType === "roundtrip" && (
          <div className="relative lg:col-span-1" ref={returnCalendarRef}>
            ...
            <button
              ...
              className="w-full bg-gray-50 border border-gray-200 rounded-sm pl-10 pr-8 py-3 text-[#111111] text-left focus:outline-none focus:border-[#AF935B] focus:ring-2 focus:ring-[#AF935B]/20 transition-all duration-300 text-sm h-[52px]"
            >
              {returnDate ? formatDisplayDate(returnDate) : <span className="text-gray-500">Return Date</span>}
            </button>
            {returnDate && (
              <button
                onClick={() => {
        ...
        <div className="relative lg:col-span-1" ref={passengerRef}>
          ...
          <button
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-sm pl-10 pr-8 py-3 text-[#111111] text-left focus:outline-none focus:border-[#AF935B] focus:ring-2 focus:ring-[#AF935B]/20 transition-all duration-300 text-sm h-[52px] flex items-center"
          >
          ...
          {showPassengerDropdown && (
            <div className="absolute bottom-full left-0 mb-1 bg-white rounded-sm shadow-lg border border-gray-200 p-4 w-56 z-[9999]">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-[#111111]">Passengers</span>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setPassengerCount(Math.max(1, passengerCount - 1))}
                    className="w-7 h-7 rounded-full bg-gray-100 hover:bg-[#AF935B]/20 flex items-center justify-center transition-colors"
                  >
                    <Minus size={14} className="text-[#725B29]" />
                  </button>
                  ...
                  <button
                    ...
                    className="w-7 h-7 rounded-full bg-gray-100 hover:bg-[#AF935B]/20 flex items-center justify-center transition-colors"
                  >
                    <Plus size={14} className="text-[#725B29]" />
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-1 flex items-center">
          <button
            ...
            disabled={isLoading}
            className={`w-full bg-gradient-to-r from-[#AF935B] to-[#725B29] text-white rounded-sm transition-all duration-300 flex items-center justify-center h-[52px] px-6 ${
              isLoading 
                ? 'opacity-80 cursor-not-allowed' 
                : 'hover:from-[#725B29] hover:to-[#AF935B] hover:scale-[1.02] active:scale-95 shadow-lg hover:shadow-[#AF935B]/30'
            }`}
          >
            {isLoading ? (
              <>
